...
Running the above script produces two key plots:

1. **Scatter Plot of \( T^2 \) vs. \( r^3 \):**  
   This plot shows a series of points that lie on (or very near) a straight line, demonstrating that \( T^2 \) is proportional to \( r^3 \).

2. **Comparison with the Theoretical Line:**  
   The second plot overlays the calculated \( T^2 \) values with the theoretical line given by:
   
   $$
   T^2 = \frac{4\pi^2}{G M} r^3.
   $$
   
   The close agreement between the two confirms the validity of Kepler's Third Law.
